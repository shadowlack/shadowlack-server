...
class SpeciesException(Exception):
    """Base exception class for species."""

    def __init__(self, msg):
        self.msg = msg


ALL_SPECIES = ('Anubi', 'Aquabat', 'Feydragon', 'Khell',
             'Lukuo', 'Pendragon', 'Takula', 'Yki')


def load_species(species):
    """Returns an instance of the named species class.
    Args:
        species (str): case-insensitive name of species to load
    Returns:
        (Species): instance of the appropriate subclass of `Species`
    """
    sp = species.capitalize()
    if sp in ALL_SPECIES:
        return globals()[sp]()
    else:
        raise SpeciesException("Invalid species specified.")


def become_species(char, species):
    """Causes a character to "transform" into the named species.
    """
    if isinstance(species, Species):
        species = species.name

    species = load_species(species)
    char.db.species = species.name

    # apply species bonuses

    # apply species languages
...
class Species():
    """Base class for species attributes"""
    ...
    @property
    def desc(self):
        """Return a formatted description of the species."""
        desc = "|g{}|n\n".format(self.name)
        if self.pronounced:
...
class Pendragon(Species):
    def __init__(self):
        ...
        # generic most "human-like" species
